...
export function Header() {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-background/95 backdrop-blur-md shadow-md border-b border-border/50" : "bg-white"}`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-14 sm:h-16 md:h-18">
        ...
        <div className="hidden lg:flex items-center gap-6 xl:gap-8 text-sm font-medium">
          <a
            href="#academic"
            className={`boty-transition ${scrolled ? "text-foreground/70 hover:text-secondary" : "text-black/80 hover:text-secondary"}`}
          >
            Academic Programmes
          </a>
          <a
        ...
        <div className="flex lg:hidden items-center gap-2">
          <a
            ...
            aria-label="Call us"
            className="p-2 animate-[ring_1.5s_ease-in-out_infinite]"
          >
          ...
          >
            {open ? (
              <X
                className={`w-6 h-6 ${scrolled ? "text-foreground" : "text-black"}`}
              />
            ) : (
              <Menu
                className={`w-6 h-6 ${scrolled ? "text-foreground" : "text-black"}`}
      ...
      {open && (
        <div className="lg:hidden bg-background border-t border-border px-4 sm:px-6 py-4 space-y-1 shadow-lg">
          <a
            href="#academic"
            className="block py-3 text-foreground hover:text-primary boty-transition"
          ...
          <a
            href="tel:08065064810"
            className="flex items-center justify-center gap-2 py-3 mt-2 rounded-full bg-white border border-secondary text-secondary font-semibold"
            onClick={() => setOpen(false)}
          >
            <img src="/Call green.svg" alt="Call" className="w-5 h-5" />
            08065064810
          ...
          <a
            href="#enquiry"
            className="block text-center py-3 mt-2 rounded-full bg-secondary text-secondary-foreground font-semibold"
            onClick={() => setOpen(false)}
          >
            Enroll Now
          </a>
